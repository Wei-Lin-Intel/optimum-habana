        ...
        **kwargs,
        ...
        if use_flash_attention and FusedSDPA:
            ...
            if q_len == 1:
                ...
                # first token
                # Currently the causal mask does not work for Mixtral MoE
...
class GaudiMixtralDecoderLayer(MixtralDecoderLayer):
    def __init__(self, config: MixtralConfig, layer_idx: int):
        ...
        self.self_attn = GaudiMixtralAttention(config=config, layer_idx=layer_idx)
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, residual, router_logits = self.post_attn_pre_moe(hidden_states, residual)
#        htcore.mark_step()

        hidden_states = self.post_moe(hidden_states, residual)
    ...
    def post_attn_pre_moe(self, hidden_states, residual):
    ...
    def post_moe(self, hidden_states, residual):
        if self.training:
            hidden_states = hidden_states + residual
        else:
            residual.add_(hidden_states)
...
class GaudiMixtralModel(MixtralModel):
    def __init__(self, config: MixtralConfig):
        ...
        self.layers = nn.ModuleList(
            [GaudiMixtralDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )

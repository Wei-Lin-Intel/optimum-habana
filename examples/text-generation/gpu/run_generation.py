...
MODEL_CLASSES = {
    ...
    "mamba": (MambaForCausalLM, GPTNeoXTokenizerFast),
}
...
def main():
    ...
    if args.batch_size > len(prompt_texts):
        num_sentences_to_add = args.batch_size - len(prompt_texts)
        for i in range(num_sentences_to_add):
            prompt_texts.append(prompt_texts[i % len(prompt_texts)])
    #import pdb;pdb.set_trace()
    # Different models need different input formatting and/or extra arguments
    requires_preprocessing = args.model_type in PREPROCESSING_FUNCTIONS.keys()
    if requires_preprocessing:
        prepare_input = PREPROCESSING_FUNCTIONS.get(args.model_type)
        preprocessed_prompt_text = prepare_input(args, model, tokenizer, prompt_texts)
    ...
    def generate_dataset(input_ids,  attention_mask):
    ...
    for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
        ...
        total_sequence = (
            prompt_texts + text[len(tokenizer.decode(encoded_prompt[0], clean_up_tokenization_spaces=True)) :]
        )

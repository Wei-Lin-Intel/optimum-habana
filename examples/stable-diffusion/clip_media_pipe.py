...
class read_image_text_from_dataset(media_ext_reader_op_impl):
    ...
    def __next__(self):
        ...
        crop_top_lefts = []
        #with torch.no_grad():
        #    model_input = vae.encode(data).latent_dist.sample()
        #    model_input = model_input * vae.config.scaling_factor
        #examples["model_input"] = model_input
        for i, x in enumerate(data):
            import pdb;pdb.set_trace()
            original_sizes.append((data.height, data.width))
            if True: #args.center_crop:
                y1 = max(0, int(round((data.height - self.dataset.image_resize) / 2.0)))
                x1 = max(0, int(round((data.width - self.dataset.image_resize) / 2.0)))
            #else:

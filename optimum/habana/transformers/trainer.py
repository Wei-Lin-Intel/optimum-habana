...
class GaudiTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # attn_softmax_bf16 and use_flash_attention is enabled only for llama, baichuan2, chatglm2, qwen1, qwen2
                if hasattr(self.model, "generation_config") and self.model.generation_config is not None:
                    if self.model.config.model_type in ["llama", "baichuan", "chatglm", "qwen", "qwen2"]:
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # attn_softmax_bf16 and use_flash_attention are enabled only for llama, baichuan2, chatglm2, qwen1, qwen2
            if hasattr(self.model, "generation_config") and self.model.generation_config is not None:
                if self.model.config.model_type in ["llama", "baichuan", "chatglm", "qwen", "qwen2"]:
                    if self.model.generation_config.attn_softmax_bf16:
                        inputs["attn_softmax_bf16"] = True

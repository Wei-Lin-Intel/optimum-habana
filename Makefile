...
DEEPSPEED_URL := git+https://github.com/HabanaAI/DeepSpeed.git@1.20.0
...
slow_tests_image_to_text_example_1x: test_installs
	python -m pytest tests/test_image_to_text_example.py -v -s -m "(not x2) and (not x8)" --token $(TOKEN)

slow_tests_image_to_text_example_2x: test_installs
	python -m pytest tests/test_image_to_text_example.py -v -s -m x2 --token $(TOKEN)
...
install_deepspeed:
	python -m pip install $(DEEPSPEED_URL)
